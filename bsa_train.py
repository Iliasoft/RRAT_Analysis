...
from bsa_headers import FILE_NAME_KEY
from bsa_data_prepare import EMBEDDINGS_FN, AUGMENTED_EMBEDDINGS_FN, AUGMENTED_EMBEDDINGS_MAP_FN, DATA_EXTRACT_FN, LABELS_KEY, LEGACY_PREDS_KEY
MODEL_FN = f"bsa_binary_classification.pth"
...
class BSA(Dataset):
    ...
    def train_test_split(root_dir, train_share, n_splits=1):
        ...
        if train_share >= 1:
            ...
            stratifier = StratifiedKFold(n_splits=n_splits, shuffle=True)
...
if __name__ == '__main__':
    ...
    lr = 0.00005 if len(sys.argv) < 6 else float(sys.argv[5]) # 0.00005 baseline
    ...
    for iteration in range(n_iterations):
        ...
        for id, dataset in enumerate(splitted_folded_datasets):
            ...
            if train_share == 1.0:
                torch.save(model_weights, os.path.join(sys.argv[1], MODEL_FN[:-4] + "_" + str(DEVICE) + MODEL_FN[-4:]))
                sys.exit(0)
